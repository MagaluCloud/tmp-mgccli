/*
*	DO NOT EDIT THIS FILE
*	IT IS AUTO GENERATED BY MGC
*	TEMPLATE: menu_item.template
 */

package clusters

import (
	"context"
	"github.com/spf13/cobra"

	"fmt"

	"github.com/magaluCloud/mgccli/beautiful"

	flags "github.com/magaluCloud/mgccli/cobra_utils/flags"

	kubernetesSdk "github.com/MagaluCloud/mgc-sdk-go/kubernetes"
)

func Update(ctx context.Context, parent *cobra.Command, clustersService kubernetesSdk.ClusterService) {

	var clusterIDFlag *flags.StrFlag //CobraFlagsDefinition

	var req_AllowedCIDRsFlag *flags.StrSliceFlag //CobraFlagsDefinition

	cmd := &cobra.Command{
		Use: "update",

		RunE: func(cmd *cobra.Command, args []string) error {

			//ServiceSDKParamCreate
			var clusterID string
			//ServiceSDKParamCreate
			var req kubernetesSdk.PatchClusterRequest

			//CobraFlagsAssign
			if len(args) > 0 {
				cmd.Flags().Set("cluster-id", args[0])
			}
			if clusterIDFlag.IsChanged() {
				clusterID = *clusterIDFlag.Value

			} else {
				return fmt.Errorf("é necessário fornecer o clusterID como argumento ou usar a flag --clusterID")
			}

			//CobraFlagsAssign
			if req_AllowedCIDRsFlag.IsChanged() {
				req.AllowedCIDRs = req_AllowedCIDRsFlag.Value

			}

			result0, err := clustersService.Update(ctx, clusterID, req)

			if err != nil {
				return err
			}

			raw, _ := cmd.Root().PersistentFlags().GetBool("raw")
			beautiful.NewOutput(raw).PrintData(result0) //PrintResult
			return nil
		},
	}

	//CobraFlagsCreation
	clusterIDFlag = flags.NewStr(cmd, "cluster-id", "", "")
	//CobraFlagsCreation
	req_AllowedCIDRsFlag = flags.NewStrSlice(cmd, "allowed-cidrs", []string{}, "")

	parent.AddCommand(cmd)

}
