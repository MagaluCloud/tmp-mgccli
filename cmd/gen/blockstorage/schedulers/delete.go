/*
*	DO NOT EDIT THIS FILE
*	IT IS AUTO GENERATED BY MGC
*	TEMPLATE: product.template
*   FILE ID: base-cli-gen/cmd/gen/blockstorage/schedulers/delete.go
*
*	CUSTOM: base-cli-custom/cmd/gen/blockstorage/schedulers/delete.go
*
 */

package schedulers

import (
	"context"

	"fmt"

	"github.com/charmbracelet/huh"

	"github.com/spf13/cobra"

	blockstorageSdk "github.com/MagaluCloud/mgc-sdk-go/blockstorage"

	flags "github.com/magaluCloud/mgccli/cobra_utils/flags"
)

func Delete(ctx context.Context, parent *cobra.Command, schedulerService blockstorageSdk.SchedulerService) {

	var idFlag *flags.StrFlag //CobraFlagsDefinition

	cmd := &cobra.Command{
		Use:   "delete [id]",
		Short: "Delete block storage volume",
		Long:  `Delete a block storage volume`,
		RunE: func(cmd *cobra.Command, args []string) error {

			var id string // ServiceSDKParamCreate

			if len(args) > 0 {
				cmd.Flags().Set("id", args[0])
			}
			if idFlag.IsChanged() {
				id = *idFlag.Value
			} else {
				return fmt.Errorf("é necessário fornecer o id como argumento ou usar a flag --id")
			} // CobraFlagsAssign

			var confirm bool
			huh.NewConfirm().Title("This action cannot be undone. Proceed?").Affirmative("Yes").Negative("No").Value(&confirm).Run()
			if !confirm {
				return nil
			}

			err := schedulerService.Delete(ctx, id)

			if err != nil {
				return err
			}

			return nil
		},
	}

	idFlag = flags.NewStr(cmd, "id", "", " (required)") //CobraFlagsCreation

	parent.AddCommand(cmd)

}
